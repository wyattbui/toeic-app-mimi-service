...
import { CreateQuestionDto, SubmitTestDto, CreateBookmarkDto } from './dto';
...
export class QuestionService {
  ...
  async getQuestionsByPart(partId: number, limit?: number) {
  ...
  async getRandomQuestions(count: number = 20, difficulty?: string) {
  ...
  async submitTest(userId: number, dto: SubmitTestDto) {
    ...
    for (const answer of dto.answers) {
      ...
      const isCorrect = correctOption?.optionLetter === answer.selectedOption;
      ...
      if (isCorrect) {
        correctAnswers++;
        if (question.part.skillType === 'Listening') {
          listeningCorrect++;
        } else {
          readingCorrect++;
        }
    ...
    const totalScore = this.calculateTOEICScore(correctAnswers, dto.answers.length);
    const listeningScore = this.calculateTOEICScore(listeningCorrect, 100); // Simplified
  ...
  async createBookmark(userId: number, dto: CreateBookmarkDto) {
    ...
    if (existingBookmark) {
      throw new ForbiddenException('Question already bookmarked');
    }
